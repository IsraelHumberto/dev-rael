...
const Side = ({ index, year, company, resume }) => {
  return (
    <div className={`mb-8 flex gap-6 sml:gap-0 sml:justify-between items-center w-full ${index%2 == 1 ? 'sml:flex-row-reverse' : ''}`}>
      ...
      <div className={`order-1 rounded-lg w-full sml:w-5/12 py-4`}>
        <h4 className="mb-1 font-bold text-accent-500">{year}</h4>
        <h3 className="mb-3 text-lg sm:text-xl">{company}</h3>
